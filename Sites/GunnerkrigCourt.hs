...
gunnerkrigCourtPageParse (WebpageReply html _) = do
   ...
   where
    ...
    comicFileName filepath url =
        ...
        in ComicTag (T.pack "gunnerkrigg_court") Nothing [UnitTag [StandAlone $ Digit chp Nothing Nothing] name UnitTagChapter] (Just $ last $ decodePathSegments $ US.fromString url)
