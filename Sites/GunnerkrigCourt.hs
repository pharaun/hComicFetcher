...
gunnerkrigCourtPageParse (WebpageReply html _) = do
   ...
   where
    ...
    comicFileName filepath url =
        ...
        in ComicTag (T.pack "gunnerkrigg_court") Nothing (Just $ UnitTag [StandAlone $ Digit chp Nothing Nothing] name) Nothing (Just $ last $ decodePathSegments $ US.fromString url)
